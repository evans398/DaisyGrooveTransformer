...
struct MidiManager {
    ...
    void HandleMidiMessage(MidiEvent midi_event)
    {
        ...
        switch(midi_event.type)
        {
            case NoteOn:
            {
                ...
                // SendMidiTest();
                // SendMidiOut(note_msg.channel, note_msg.note, note_msg.velocity);
    ...
    void SendMidiTest(bool note_on_off) {
        ...
        uint8_t data_on[3] = { 0 };
        uint8_t data_off[3] = { 0 };

        //** NOTE ON*//
        data_on[0] = (channel & 0x0F) + 0x90;  // limit channel byte, add status byte
        data_on[1] = note & 0x7F;              // remove MSB on data
        data_on[2] = velocity & 0x7F;
        usb_midi->SendMessage(data_on, 3);

        //** NOTE OFF*//
        data_off[0] = (channel & 0x0F) + 0x80;  // limit channel byte, add status byte
        data_off[1] = note & 0x7F; 
        data_off[2] = velocity_off & 0x7F;
        usb_midi->SendMessage(data_off, 3); 
    }

    void SendMidiOut(int channel, uint8_t note, uint8_t velocity) {
